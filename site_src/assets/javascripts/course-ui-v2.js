(() => {
  ...
  const initializeCourseNav = () => {
    let button = document.querySelector(".course-nav-toggle-v2");

    if (!button) {
      button = document.createElement("button");
      button.type = "button";
      button.className = "course-nav-toggle-v2";
      ...
      button.addEventListener("click", () => {
        ...
        localStorage.setItem(
          ...
          document.body.classList.contains("course-nav-collapsed") ? "true" : "false"
        );
        updateToggle(button);
      });
    }
